...
public class Spoofing extends SettingsPreferenceFragment implements
    ...
    public static void reset(Context mContext) {
        ...
        Settings.Secure.putIntForUser(resolver,
                PI_PHOTOS_SPOOF, 1, UserHandle.USER_CURRENT);
        Settings.Secure.putIntForUser(resolver,
                PI_SNAPCHAT_SPOOF, 0, UserHandle.USER_CURRENT);
        Settings.Secure.putIntForUser(resolver,
                Settings.Secure.PI_TENSOR_SPOOF, 0, UserHandle.USER_CURRENT);
        Settings.Secure.putIntForUser(resolver,
                Settings.Secure.PER_APPS_DEVICE_SPOOF_ENABLED, 0, UserHandle.USER_CURRENT);
        Settings.Secure.putStringForUser(resolver,
                Settings.Secure.PER_APPS_DEVICE_SPOOF, "", UserHandle.USER_CURRENT);
        Settings.Secure.putStringForUser(resolver,
                Settings.Secure.PER_APPS_DEVICE_SPOOF_CACHE, "", UserHandle.USER_CURRENT);
        Settings.Secure.putStringForUser(resolver,
                Settings.Secure.CUSTOM_SPOOF_PROFILES, "", UserHandle.USER_CURRENT);
        PlayIntegrityFix.reset(mContext);
        TrickyStore.reset(mContext);
    }
